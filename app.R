 ...
 # Create map plot
  ...
  output$mapPlot <- renderLeaflet({
    map <- leaflet(polygon_data()) %>%
      ...
      addMarkers(layerId = paste0("Lot ",points_data()$LOT,", ",points_data()$ADDRESS),lng=as.numeric(points_data()$LONGITUDE), lat=as.numeric(points_data()$LATITUDE),label = paste0("Lot ",points_data()$LOT,", ",points_data()$ADDRESS)) %>% 
      addPolygons(layerId = ~LOTPLAN,fillOpacity = 1, weight = 2, smoothFactor = 0.5,opacity = 1.0,fillColor = "green",highlightOptions = highlightOptions(fillColor = "yellow"))
    map
  })
